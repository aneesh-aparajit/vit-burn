...
impl<B: Backend> PositionalEmbedding<B> {
    pub fn new(
        ...
        dropout: f64,
        device: &B::Device,
    ) -> PositionalEmbedding<B> {
        let position_ix: Tensor<B, 1, Float> = Tensor::arange(
            Range {
                start: 0,
                end: max_token_length as i64,
            },
            device,
        )
        .float();
        ...
        let even_embeds = position_ix
            .clone()
            .div(
                denominator
                    .clone()
                    .powf_scalar(2.0 * (ix as f32) / (embedding_dim as f32)),
            )
            .sin()
            .clone()
            .reshape([max_token_length, 1]);

        let odd_embeds = position_ix
            .clone()
            .div(
                denominator
                    .clone()
    ...
    pub fn forward(&self, inputs: Tensor<B, 3>) -> Tensor<B, 3> {
        ...
        for ix in 1..batch_size {
            let y = inputs
                ...
                .slice([ix..ix + 1])
                .squeeze(0)
                .add(self.positional_embedding_matrix.clone().slice([ix..ix + 1]))
                .reshape([1, seq_len, embedding_dim]);
